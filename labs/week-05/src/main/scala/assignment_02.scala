...
//ds.show(5,false)
 val schema = new StructType()
    ...
    //q3
   // var minMaxRow=df_with_schema.select(min("temp").alias("min_temp"),max("temp").alias("max_temp"),min("battery_le
vel").alias("min_battery"),max("battery_level").alias("max_battery"),min("c02_level").alias("min_c02"),max("c02_level
").alias("max_c02"),min("humidity").alias("min_humidity"),max("humidity").alias("max_humidity"))
